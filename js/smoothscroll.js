(function () {
    ...
    var s = {left: 37, up: 38, right: 39, down: 40, spacebar: 32, pageup: 33, pagedown: 34, end: 35, home: 36};
    var y = M;
    ...
    function P() {
        ...
        if (top != self) {
            ...
            if (ac > U && (aa.offsetHeight <= U || Z.offsetHeight <= U)) {
                ...
                Y.style.cssText = "position:absolute; z-index:-10000; top:0; left:0; right:0; height:" + A.scrollHeight + "px";
    ...
    function Q(Z, ab, aa) {
        ...
        H.push({x: ab, y: aa, lastX: (ab < 0) ? 0.99 : -0.99, lastY: (aa < 0) ? 0.99 : -0.99, start: Date.now()});
        if (h) {
            return
        }
    ...
    function n(X) {
        ...
        if (m(e, "embed") || (m(Y, "embed") && /\.pdf/i.test(Y.src)) || m(e, "object")) {
            return true
        }
    ...
    function x(V) {
        var ac = V.target;
        var aa = V.ctrlKey || V.altKey || V.metaKey || (V.shiftKey && V.keyCode !== s.spacebar);
        if (!document.contains(e)) {
            e = document.activeElement
        }
        var W = /^(textarea|select|embed|object)$/i;
        var X = /^(button|submit|radio|checkbox|file|color|image)$/i;
        if (W.test(ac.nodeName) || m(ac, "input") && !X.test(ac.type) || m(e, "video") || q(V) || ac.isContentEditable || V.defaultPrevented || aa) {
            return true
        }
        if ((m(ac, "button") || m(ac, "input") && X.test(ac.type)) && V.keyCode === s.spacebar) {
            return true
        }
        var Y, ae = 0, ad = 0;
        var Z = J(e);
    ...
    var G = (function () {
        return (window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function (W, V, U) {
